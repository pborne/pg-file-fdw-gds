...
static const struct FileFdwOption valid_options[] = {
	...
	{"hdfs_directory_path", ForeignTableRelationId},
...
static bool check_selective_binary_conversion(RelOptInfo *baserel,
								  Oid foreigntableid,
								  List **columns);
static bool fileNextCopyFrom(CopyState cstate, ExprContext *econtext,
...
file_fdw_validator(PG_FUNCTION_ARGS)
{
	...
	foreach(cell, options_list)
	{
		...
		else if (strcmp(def->defname, "hdfs_directory_path") == 0)
		{
			if (hdfs_directory_path)
				ereport(ERROR,
						(errcode(ERRCODE_SYNTAX_ERROR),
						 errmsg("conflicting or redundant options")));
			hdfs_directory_path = defGetString(def);
		}
			   ...
			   char **filename, List **other_options)
{
	...
	/*
	 * Separate out the filename and hdfs directory path from options.
	 */
	*filename = NULL;
	prev = NULL;
	for (listCell = list_head(options); listCell != NULL; listCell = next)
	{
		DefElem    *def = (DefElem *) lfirst(listCell);
		...
		{
			*filename = defGetString(def);
			options = list_delete_cell(options, listCell, prev);
		}
		else if (strcmp(def->defname, "hdfs_directory_path") == 0)
		{
			options = list_delete_cell(options, listCell, prev);
...
fileIterateForeignScan(ForeignScanState *node)
{
	 ...
	 */
	ExecClearTuple(slot);
	found = fileNextCopyFrom(festate->cstate, NULL,
...
/*
 * fileNextCopyFrom wraps around the NextCopyFrom() function. The function skips
 * malformed lines similar to Hive's deserializers and makes sure that we return
 * the next valid tuple from the foreign file.
 */
static bool
fileNextCopyFrom(CopyState cstate, ExprContext *econtext,
				 Datum *values, bool *nulls, Oid *tupleOid)
{
	MemoryContext currentContext = CurrentMemoryContext;
	bool nextTupleFound = true;
	bool nextTupleValid = false;

	/*
	 * If we are reading the first line of a CSV file, then we assume that we
	 * may have spilled bytes from the previous HDFS block, and skip it. XXX:
	 * The spilling bytes issue will be fixed in CitusDB v2.1; and this check
	 * that uses internal copy state and the line skipping logic will go away.
	 */
	if (cstate->cur_lineno == 0 && cstate->csv_mode && !cstate->header_line)
	{
		/* revert to text mode to skip quotes and read until newline */
		cstate->csv_mode = false;

		PG_TRY();
		{
			bool found = NextCopyFrom(cstate, econtext, values, nulls, tupleOid);
			(void) found;
		}
		PG_CATCH();
		{
			MemoryContext errorContext = MemoryContextSwitchTo(currentContext);
			ErrorData *errorData = CopyErrorData();
			FlushErrorState();

			ereport(DEBUG1, (errmsg("skipping first line in CSV file"),
							 errcontext("%s", errorData->context)));
	...
	/*
	 * We keep looping as long as we haven't reached the end of file, and until
	 * we find a valid line.
	 */
	while (nextTupleFound && !nextTupleValid)
	{
		PG_TRY();
		{
			nextTupleFound = NextCopyFrom(cstate, econtext, values, nulls, tupleOid);
			nextTupleValid = true;
		}
		PG_CATCH();
		{
			...
			/* downgrade error to debug message, and ereport */
			ereport(DEBUG1, (errcode(errorData->sqlerrcode),
							 errmsg("%s", errorData->message),
							 errcontext("%s", errorData->context)));
						 ...
						 double *totalrows, double *totaldeadrows)
{
	...
	*totaldeadrows = 0;
	for (;;)
	{
		...
		found = fileNextCopyFrom(cstate, NULL, values, nulls, NULL);
